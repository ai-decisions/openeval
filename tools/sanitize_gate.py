...
CONTENT_PATTERNS: list[tuple[str, re.Pattern[str]]] = [
    ...
    # Unlike sibling repositories, openeval deliberately SHIPS a BigQuery
    # public-dataset path (scripts/bq_first_seen.py), so "google.cloud" /
    # "BigQuery" are legitimate content here and are NOT banned. Private
    # GCP project ids are still caught by the private ruleset, and gs://
    # URIs by the generic pattern above.
]
