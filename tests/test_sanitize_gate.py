...
class TestRepoOverrides:
    """This repository ships tools/sanitize_overrides.py disabling the
    private-stack-marker pattern: the published method legitimately runs
    against the BigQuery public dataset (scripts/bq_first_seen.py)."""

    def test_bigquery_is_legitimate_content_here(self, tmp_path: Path) -> None:
        tree = tmp_path / "tree"
        tree.mkdir()
        (tree / "probe.py").write_text(
            'DATASET = "bigquery-public-data.crypto_ethereum"\n'
            "# google.cloud client is imported by the public runner\n"
        )
        r = _run("--public-mode", "--root", str(tree))
        assert r.returncode == 0, r.stdout

    def test_every_other_pattern_still_applies(self, tmp_path: Path) -> None:
        tree = tmp_path / "tree"
        tree.mkdir()
        gcs = "".join(map(chr, (103, 115, 58, 47, 47))) + "bucket/x"
        (tree / "probe.py").write_text(f'a = "{gcs}"\n')
        r = _run("--public-mode", "--root", str(tree))
        assert r.returncode == 1, r.stdout
        assert "gcs-uri" in r.stdout
